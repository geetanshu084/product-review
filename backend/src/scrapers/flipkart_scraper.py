...
class FlipkartScraper(BaseScraper):
    ...
    def get_cache_key(self, product_id: str) -> str:
        """Generate Redis cache key for Flipkart product"""
        return f"product:flipkart:{product_id}"
    ...
    def scrape_product(self, url: str) -> Dict:
        ...
        # Check cache first
        cached_data = self._get_from_cache(product_id)
        if cached_data:
            print(f"✓ Using cached data for Product ID: {product_id} (from Redis)")
            return cached_data
        ...
        try:
            ...
            # Save to cache
            self._save_to_cache(product_id, product_data)
            print(f"✓ Cached data for Product ID: {product_id} (TTL: 24 hours)")
    ...
    def _save_to_cache(self, product_id: str, product_data: Dict):
